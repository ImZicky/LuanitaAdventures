...
import { useCommonService } from "./commonService";
...
export function usePokemonService() {
  ...
  return {
    ...
    getPokemonForBattle : async (pokemonApi: Pokemon, playersPokemonLevel: number, nickname: string | undefined, forceShiny ?: boolean, isPlayerPokemon?: boolean ) => {
      let pokemonLevel: number;

      if (isPlayerPokemon) {
        pokemonLevel = Math.floor(Math.random() * (80 - 60 + 1)) + 60; // Gera um nível entre 60 e 80
      } else {
        pokemonLevel = pokemonService.getRamdomLevelBasedOnPlayer(playersPokemonLevel);
      }

      // Definição dos ataques manuais
      const manualAttacks: { [key: string]: string[] } = {
        'lucario': ['Aura Sphere', 'Close Combat', 'Meteor Mash', 'Swords Dance'],
        'gengar': ['Shadow Ball', 'Hypnosis', 'Dream Eater', 'Lick'],
        'charizard': ['Flamethrower', 'Blast Burn', 'Wing Attack', 'Dragon Claw'],
        'greninja': ['Water Shuriken', 'Hydro Pump', 'Dark Pulse', 'Ice Beam'],
        'decidueye': ['Spirit Shackle', 'Leaf Blade', 'Shadow Sneak', 'Brave Bird'],
        'buizel': ['Aqua Jet', 'Water Pulse', 'Ice Punch', 'Sonic Boom']
      };

      let skills: PokemonForBattleSkills[];
      const pokemonName = pokemonApi.name.toLowerCase();

      if (manualAttacks[pokemonName]) {
        skills = manualAttacks[pokemonName].map(attackName => ({
          name: attackName,
          damage: 20, // Dano padrão
          ppNow: 10, // PP padrão
          ppTotal: 10 // PP total padrão
        }));
      } else {
